...
Последствия: UI и документация не должны обещать стопроцентную phishing protection; результат является предупреждением, а не вердиктом безопасности. Link Safety не перехватывает same-origin ссылки: внутреннюю SPA-навигацию обрабатывает сам сайт, а проверка применяется к переходам за пределы текущего origin.
